package com.trump.auction.pals.util.thread;
...
public class ThreadPool2 extends ThreadGroup {
	...
	private static ThreadPool2 pool;

	private ThreadPool2(int poolSize) { // poolSize 表示线程池中的工作线程的数量
	...
	private synchronized Runnable getTask(int threadid) throws InterruptedException {
		while (workQueue.size() == 0) {
			if (isClosed)
				return null;
			System.out.println("pool2工作线程" + threadid + "等待任务...");
			wait(); // 如果工作队列中没有任务,就等待任务
		}
		System.out.println("pool2工作线程" + threadid + "开始执行任务...");
	...
	/**
	 * 内部类,pool2工作线程,负责从工作队列中取出任务,并执行
	 * 
	 * @author sunnylocus
	 */
