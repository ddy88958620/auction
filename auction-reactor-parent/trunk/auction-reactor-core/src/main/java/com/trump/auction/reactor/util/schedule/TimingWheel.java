...
/**
 * A timing wheel data structures to efficiently implement a timer facility,
 * such as I/O timeout scheduling.<br>
 * {@link TimingWheel} creates a new thread whenever it is instantiated and
 * started, so don't create many instances.
 * <p>
 ...
 * </pre>
 * 
 * After expiration occurs, the {@link ExpirationListener} interface will be
 * invoked and the expired object will be the argument for callback method
 * {@link ExpirationListener#expired(Object)}
 * <p>
 * As timing-wheel use map structure internal, so any element added to
 * timing-wheel should implement its own <tt>equals(o)</tt> and
 * <tt>hashCode()</tt> method.
 * <p>
 * {@link TimingWheel} is based on
 * <a href="http://cseweb.ucsd.edu/users/varghese/">George Varghese</a> and Tony
 * Lauck's paper,
 * <a href="http://cseweb.ucsd.edu/users/varghese/PAPERS/twheel.ps.Z">'Hashed
 * and Hierarchical Timing Wheels: data structures to efficiently implement a
 * timer facility'</a>. More comprehensive slides are located <a href=
 * "http://www.cse.wustl.edu/~cdgill/courses/cs6874/TimingWheels.ppt">here</a>.
 * 
 * @author mindwind
...
public class TimingWheel<E> {
	...
	protected final ArrayList<Slot<E>> wheel;

	@Getter(AccessLevel.PROTECTED)
	protected final Map<E, Slot<E>> indicator = new ConcurrentHashMap<E, Slot<E>>();
	...
	@Getter(AccessLevel.PROTECTED)
	protected final CopyOnWriteArrayList<ExpirationListener<E>> expirationListeners = new CopyOnWriteArrayList<ExpirationListener<E>>();

	protected volatile int currentTickIndex = 0;
	...
	/**
	 * Add a element to {@link TimingWheel} and start to count down its
	 * life-time.
	 * 
	 * @param e
	 ...
	 */
	public long add(E e) {
		// at any time just only one e(element) in the timing-wheel, all
		// operations(add,remove,put) on this element should be synchronized.
		synchronized (e) {
			checkAdd(e);
	...
	private class TickWorker implements Runnable {
		...
		private void waitForNextTick() {
			for (;;) {
				...
				// log.debug("[CRAFT-ATOM-UTIL] Wait for next tick sleep
				// |sleepTime={}|", sleepTime);
