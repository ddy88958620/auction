...
<form id="form" class="layui-form  layui-form-pane1 pzjzsj" method="POST" style="margin-left: 550px;">
    ...
    <fieldset class="layui-elem-field layui-field-title"  style="margin-top: 20px;width: 800px;">
        <legend style="margin-bottom: 20px;"><strong>新增消息信息</strong></legend>
            ...
            <div class="layui-input-block" style="width: 100px;">
                <select class="layui-input" name="channelType" id="channelType" autocomplete="off">
                    <c:forEach var="item" items="${messageTypeList}">
                        <option value="${item.help_type}" <c:if test="${item.help_type eq params.channelType}">selected="selected"</c:if>>${item.help_str}</option>
                    </c:forEach>
                </select>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline" style="width: 700px;">
                <textarea id="contentTxt" name="contentTxt" style="display: none;">
                </textarea>
            </div>
        </div>
...
<script>
    ...
    layui.use(['form','upload','layedit'], function () {
        ...
        form.on('submit(submitBtn)', function (data) {
            ...
            $.post("saveMessageCenter", data.field,
                function (data) {
                    if (data.code == '0') {
