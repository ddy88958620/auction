            ...
            <div class="layui-input-inline" style="width: 100px;">
                    <select name="merchantType" lay-verify="required">
                        <c:forEach var="item" items="${merchantType}">
                            <option value="${item.key}" <c:if test="${item.key eq merchantInfo.merchantType}">selected="selected"</c:if>>${item.value}</option>
                        </c:forEach>
                    </select>
            </div>
    </div>
        ...
        <div class="layui-input-inline" style="width: 100px;">
            <select name="status" lay-verify="required">
                <c:forEach var="item" items="${merchantStatus}">
                    <option value="${item.key}" <c:if test="${item.key eq merchantInfo.status}">selected="selected"</c:if>>${item.value}</option>
                </c:forEach>
            </select>
        </div>
    </div>
