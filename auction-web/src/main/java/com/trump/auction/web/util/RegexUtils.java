...
public class RegexUtils {
    ...
    /**
     * 匹配银行卡 第一位非0的15位或19位
     * @param bankCardNo 银行卡好
     * @return 验证成功返回true，验证失败返回false
     */
    public static boolean checkBankCardNo(String bankCardNo) {
        return Pattern.matches("^([1-9]{1})(\\d{14}|\\d{15}|\\d{18})$", bankCardNo);
    }

    /**
     * 是否是合法信用卡号
     * http://blog.csdn.net/ywt_go/article/details/7730380
     * @param creditCardNo 信用卡号
     * @return 验证成功返回true，验证失败返回false
     */
    public static boolean checkCreditCardNo(String creditCardNo) {
        if (creditCardNo == null) return false;
        int sumOdd = 0;
        int sumEven = 0;
        int length = creditCardNo.length();
        int[] wei = new int[length];
        for (int i = 0; i < creditCardNo.length(); i++) {
            wei[i] = Integer.parseInt(creditCardNo.substring(length - i - 1, length - i));// 从最末一位开始提取，每一位上的数值
    ...
    public static void main(String[] args) {
    	System.out.println(checkPassword("111ddd"));
	}
}
